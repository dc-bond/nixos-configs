...
15. **Comments carry only what the code cannot** - Keep strictly to the set of facts a reader cannot derive from the code in front of them: a constraint, a gotcha, an upstream quirk, the non-obvious reason a value was chosen. No editorializing, no historical commentary, no before/after comparisons with whatever the code replaced, no narrating how a value was arrived at. Comments describe the code **as it stands now** - when you change code, rewrite its comments to match the new state rather than layering new notes on top of stale ones. Same-line comment for a single line, a short block above a block; lowercase and terse. Rationale needing more than a sentence or two belongs in the commit message or `DEVIATIONS.md`, not the source.
