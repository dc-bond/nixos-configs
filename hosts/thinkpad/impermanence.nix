...

{
  ...
  environment.persistence."/persist" = {
    ...
    directories = [
      ...
      "/var/log/journal" # journald storage is "persistent" (foundation.nix); without this bind mount it lands on the tmpfs root, so logs are lost on reboot AND SystemMaxUse=2G is charged against 7.4 GiB of RAM
    ];
    ...
    users.chris = {
      files = [
        ...
        # .claude/settings.json is now declarative (home-manager symlinks it read-only from the store — see home-manager/chris/claude-code.nix); do not persist it
      ];
      directories = [
  ...
  systemd.tmpfiles.rules = [
    "d /persist/var/lib/private 0700 root root -" # systemd requires mode 0700 for StateDirectory parent when using DynamicUser, must fix /persist since will be bind mounted to /var/lib/private
