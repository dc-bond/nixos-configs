...

{
  ...
  disko.devices = {
    disk = {
      main = {
        type = "disk";
        device = "/dev/disk/by-id/ata-512GB_SSD_MP32B12203488"; # 512GB SATA SSD
        #device = "/dev/sda"; # hardware path
        content = {
          type = "gpt";
          partitions = {
  ...
  services = {
    ...
    btrfs.autoScrub = {
      ...
      fileSystems = [ "/dev/disk/by-id/ata-512GB_SSD_MP32B12203488-part2" ];
    };
  ...
  #      BTRFS_ROOT=$(mktemp -d)
  #      ${pkgs.util-linux}/bin/mount -t btrfs -o subvolid=5 /dev/disk/by-id/ata-512GB_SSD_MP32B12203488-part2 "$BTRFS_ROOT"
