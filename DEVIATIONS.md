...
| `nixos-system/unifi.nix` | `mongodb` | `mongodb-ce` overridden to 7.0.40, in place of the module default `pkgs.mongodb-7_0` | MongoDB is SSPL, so Hydra builds no MongoDB at all and nothing in nixpkgs has a binary substitute — `mongodb-7_0` compiles from source for 3-5h on aspen and wants ~15G at the `mongod` link, repeating on every version bump. `mongodb-ce` is the same server from upstream's prebuilt tarball (`fetchurl` + `autoPatchelfHook`), pinned back off 8.2 to the 7.0 unifi expects. Ships `mongod`/`mongos` but not the legacy `mongo` shell, which unifi does not use | nixpkgs gains a cached or prebuilt `mongodb-7_0`, or `services.unifi` grows a prebuilt option |
