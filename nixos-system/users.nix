{ 
  pkgs, 
  config, 
  ... 
}: 

{

  sops.secrets.chrisPasswd.neededForUsers = true;

  security.sudo.wheelNeedsPassword = false;
  
  users.users = {
    
    chris = { # all hosts
      isNormalUser = true;
      hashedPasswordFile = config.sops.secrets.chrisPasswd.path; # create hashed password with 'echo "password" | mkpasswd -s'
      extraGroups = [
        "wheel" 
        "i2c" # for controlling i2c/ddcutil, only if ddcutil is installed
      ];
      shell = pkgs.zsh; # user-specific z-shell configs in home.nix
      openssh.authorizedKeys.keys = [ 
        "ssh-rsa  openpgp:0xE5DCB627"
      ];
    };

    danielle = { # only on lillypad
      isNormalUser = true;
      hashedPasswordFile = config.sops.secrets.daniellePasswd.path;
      extraGroups = [
        "wheel" 
        "i2c" # for controlling i2c, only if i2c-tools is installed in host's configuration.nix
      ];
      shell = pkgs.zsh; # user-specific z-shell configs in home.nix
      openssh.authorizedKeys.keys = [ 
        "ssh-rsa  openpgp:0xE5DCB627"
      ];
    };

    root = { # all hosts
      shell = pkgs.zsh;
    };

  };

}

{ 
  pkgs, 
  config,
  lib,
  ... 
}: 

{

  sops.secrets = {
    chrisPasswd.neededForUsers = true;
  #} // lib.optionalAttrs (config.networking.hostName == "lillypad") {
  #  daniellePasswd.neededForUsers = true;
  };

  security.sudo.wheelNeedsPassword = false;
  
  users.users = {
    
    chris = { # chris user on all hosts
      isNormalUser = true;
      hashedPasswordFile = config.sops.secrets.chrisPasswd.path;
      extraGroups = [ "wheel" ] 
      ++ lib.optional (lib.elem config.networking.hostName ["cypress" "thinkpad"]) "i2c";
      shell = pkgs.zsh;
      openssh.authorizedKeys.keys = [ 
        "ssh-rsa  openpgp:0xE5DCB627"
      ];
    };

    root = { # root user on all hosts
      shell = pkgs.zsh;
    };

  #} // lib.optionalAttrs (config.networking.hostName == "lillypad") {
  #  danielle = {
  #    isNormalUser = true;
  #    hashedPasswordFile = config.sops.secrets.daniellePasswd.path;
  #    extraGroups = [ "wheel" ] 
  #    ++ lib.optional (lib.elem config.networking.hostName ["lillypad"]) "i2c";
  #    shell = pkgs.zsh;
  #    openssh.authorizedKeys.keys = [ 
  #      "ssh-rsa  openpgp:0xE5DCB627"
  #    ];
  #  };

  };

}