...

{
  ...
  virtualisation.oci-containers.containers = {
    ...
    "${app1}" = {
      ...
      environment = { 
        HOST = "0.0.0.0";
        MONGO_URI = "mongodb://${app2}:27017/LibreChat";
        MEILI_HOST = "http://${app3}:7700";
        RAG_PORT = "\${RAG_PORT:-8000}";
        RAG_API_URL = "http://rag_api:\${RAG_PORT:-8000}";
      };
      extraOptions = [
        "--network=${app}"
        "--ip=${configVars.librechatProxyIp}"
        "--tty=true"
        "--stop-signal=SIGINT"
      ];
      labels = {
    ...
    "${app5}" = {
      ...
      environmentFiles = [ config.sops.templates."${app5}-env".path ];
      environment = { 
