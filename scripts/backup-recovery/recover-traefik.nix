...
let
  ...
  recoverTraefikScript = pkgs.writeShellScriptBin "recoverTraefik" ''
   ...
   { set +x; log "starting backup recovery for traefik on $HOST"; } 2>/dev/null

   { set +x; log "changing directory to ${config.backups.borgDir}"; } 2>/dev/null
   dc
   cd ${config.backups.borgDir}

   { set +x; log "extracting application data"; } 2>/dev/null
   sudo -E ${pkgs.borgbackup}/bin/borg extract --verbose --list ${config.backups.borgDir}/$HOST::$ARCHIVE var/lib/traefik --strip-components 2
   ...
   { set +x; log "stopping traefik service on $HOST"; } 2>/dev/null
   ssh $HOST 'sudo systemctl stop traefik.service'

   { set +x; log "removing existing application data on $HOST"; } 2>/dev/null
   ssh $HOST 'sudo rm -rf /var/lib/traefik'

   { set +x; log "transferring restored data to $HOST"; } 2>/dev/null
   rsync --progress -avzh ${config.backups.borgDir}/traefik $HOST:/tmp
   ssh $HOST 'sudo mv /tmp/traefik /var/lib'
   
   { set +x; log "changing ownership of restored application data"; } 2>/dev/null
   ssh $HOST 'sudo chown -R traefik:traefik /var/lib/traefik'

   { set +x; log "cleaning up local restore directory"; } 2>/dev/null
   sudo rm -rf ${config.backups.borgDir}/traefik

   { set +x; log "restarting restored traefik service on $HOST"; } 2>/dev/null
   ssh $HOST 'sudo systemctl start traefik.service'
   '';
